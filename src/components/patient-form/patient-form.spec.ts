...
describe('<cosmo-patient-form/> spec', () => {
  ...
  it('should dispatch an update event if there is a patient as input', async () => {
    ...
    instance.patient = {firstname: 'first', lastname: 'last', id: generateId(), createdAt: Date.now()};
  ...
  it('should dispatch an update event with the corresponding formvalue', async () => {
    ...
    instance.patient = {firstname: 'first', lastname: 'last', id: id, createdAt: now};
    const event = await instance.handleSubmit();
    expect(event.value).toBe({firstname: 'first', lastname: 'last', id: id, createdAt: now});
  });
